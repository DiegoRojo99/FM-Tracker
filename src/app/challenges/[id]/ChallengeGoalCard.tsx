...
type ChallengeGoalCardProps = {
  ...
  userChallenge: CareerChallengeWithDetails | null;
};

export default function ChallengeGoalCard({ goal, userChallenge }: ChallengeGoalCardProps) {
  const userChallengeGoal = userChallenge?.goalProgress.find((gp: CareerChallengeGoal) => gp.challengeGoalId === goal.id);
  const isCompleted = userChallengeGoal?.isComplete;
  
  return (
    <div 
      key={goal.id} 
      className={`flex items-center gap-4 p-4 rounded-lg transition-all duration-200 border-l-4 ${
        isCompleted 
