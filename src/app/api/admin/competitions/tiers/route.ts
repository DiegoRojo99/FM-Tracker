...
export async function GET(req: NextRequest) {
  return withAuth(req, async () => {
    try {
      ...
      console.error('Admin competition GET failed:', error);
      return apiError(error instanceof Error ? error.message : 'Failed to load competitions', 500);
    }
  }, { requireAdmin: true });
}
...
export async function PATCH(req: NextRequest) {
  return withAuth(req, async () => {
    try {
      ...
      const { id, tier, type, isActive, isFemale } = body;
      ...
      if (isFemale !== undefined && typeof isFemale !== 'boolean') {
        return badRequest('isFemale must be a boolean');
      }
      ...
      const updated = await prisma.competitionGroup.update({
        where: { id },
        data,
        select: { id: true, name: true, tier: true, type: true, isActive: true, isFemale: true },
      });

      return ok(updated);
    } catch (error) {
      console.error('Admin competition PATCH failed:', error);
      return apiError(error instanceof Error ? error.message : 'Failed to update competition', 500);
    }
  }, { requireAdmin: true });
}
