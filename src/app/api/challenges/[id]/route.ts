...
import { getChallengeByKey, getUserChallengesByChallengeKey } from '@/lib/db/challenges';
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withOptionalAuth(req, async (uid) => {
    ...
    try {
      ...
      if (uid) {
        userChallenges = await getUserChallengesByChallengeKey(challengeKey, uid);
      }
